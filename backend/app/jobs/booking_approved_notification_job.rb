...
class BookingApprovedNotificationJob < ApplicationJob
  queue_as :default
  ...
  def perform(booking_id, recipient_id)
    ...
    InAppNotification.create_notification(
      ...
      message: "Your booking with #{vendor_name} has been confirmed",
    ...
    Rails.logger.info "Booking approved notification sent for booking #{booking_id} to user #{recipient_id}"
  end
end
